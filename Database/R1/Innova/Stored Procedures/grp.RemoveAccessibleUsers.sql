...
IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'grp.RemoveAccessibleUsers'))
	DROP PROCEDURE [grp].[RemoveAccessibleUsers]
GO

CREATE PROC [grp].[RemoveAccessibleUsers]
(
	@groupId INT,
...
IF EXISTS( SELECT * FROM sys.database_principals WHERE name = 'svc_innova_qa' )
    GRANT EXECUTE ON [grp].[RemoveAccessibleUsers] TO svc_Innova_qa
GO